%% ========================================================================
% Modified version of the spm_figure.m. Modifications include changes in
% size and shape of the window and a slightly modified UI.
...
function varargout = bayinf_figure(varargin)
...
F = bayinf_figure('CreateWin',Tag,Name,Visible);
bayinf_figure('CreateBar',F);
...
F = bayinf_figure('FindWin',Tag);

if isempty(F)
    if ischar(Tag)
        switch Tag
            case 'Graphics'
                F = bayinf_figure('Create','Graphics','Graphics');
            case 'DEM'
                F = bayinf_figure('Create','DEM','Dynamic Expectation Maximisation');
            case 'DFP'
                F = bayinf_figure('Create','DFP','Variational filtering');
            case 'FMIN'
                F = bayinf_figure('Create','FMIN','Function minimisation');
            case 'MFM'
                F = bayinf_figure('Create','MFM','Mean-field and neural mass models');
            case 'MVB'
                F = bayinf_figure('Create','MVB','Multivariate Bayes');
            case 'SI'
                F = bayinf_figure('Create','SI','System Identification');
            case 'PPI'
                F = bayinf_figure('Create','PPI','Physio/Psycho-Physiologic Interaction');
            case 'Interactive'
                F = spm('CreateIntWin');
            case 'Satellite'
                F = bayinf_figure('CreateSatWin');
            otherwise
                F = bayinf_figure('Create',Tag,Tag);
        end
    end
else
    if strcmpi(get(F,'Visible'),'on')
        bayinf_figure('Focus',F);
    else
        bayinf_figure('Select',F);
    end
end
varargout = {F};
...
hMenu = bayinf_figure('FindWin','Menu');
...
if ~isempty(F), F = bayinf_figure('FindWin',F); end
...
bayinf_figure('Print',F,fname,pf(fi));
...
% MODIFICATION: Different size & position as well as name
F    = figure(...
    'MenuBar','none',...
    'Tag',Tag,...
    'Name','Visualization',...
    'Position',[(scrsz(3)* 0.6 -900/2) (scrsz(4)* 0.7-325/2) 600 320],...
    'Resize','off',...
    'Color','w',...
    'ColorMap',gray(64),...
    'DefaultTextColor','k',...
    ...
    'DefaultUicontrolFontSize',FS(14),...
...
isdocked = strcmp(get(bayinf_figure('FindWin','Menu'),'WindowStyle'),'docked');
if isdocked
    try
        desktop = com.mathworks.mde.desk.MLDesktop.getInstance;
        group   = ['Statistical Parametric Mapping (' spm('Ver') ')'];
...
F = bayinf_figure('FindWin','Satellite');
if ~isempty(F)
    bayinf_figure('Focus', F)
else
    FS   = spm('FontSizes');             %-Scaled font sizes
...
uimenu(t0,'Label','BPI Help','CallBack','bpi_help');
...
if nargin<2
    ...
    F = bayinf_figure('FindWin',varargin{2});
...
h  = [get(0,'CurrentFigure') bayinf_figure('FindWin','Satellite')];
...
SatWindow = bayinf_figure('FindWin','Satellite');
...
hMenu = bayinf_figure('FindWin','Menu');
hInt  = bayinf_figure('FindWin','Interactive');
hGra  = bayinf_figure('FindWin','Graphics');
h     = [hMenu hInt hGra];
group   = ['Statistical Parametric Mapping (' spm('Ver') ')'];
try
    desktop = com.mathworks.mde.desk.MLDesktop.getInstance;
...
hInt  = bayinf_figure('FindWin','Interactive');
hGra  = bayinf_figure('FindWin','Graphics');
hSat  = bayinf_figure('FindWin','Satellite');
hBat  = bayinf_figure('FindWin','cfg_ui');
